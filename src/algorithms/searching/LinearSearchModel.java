...
public class LinearSearchModel extends SearchModel {
    ...
    public LinearSearchModel() { super("Linear Search", "O(n)"); }

    @Override
    public void step() {
        if (isDone() || current >= array.length) { markDone(); return; }
        addComparison();
        highlight = new int[]{current};
        visited.add(current);
        searchPath = visited.stream().mapToInt(Integer::intValue).toArray();
        if (array[current] == target) {
            foundIndex = current; markDone();
        } else {
            current++;
    ...
    public void resetState(int[] a) {
        ...
        current = 0; foundIndex = -1;
        visited.clear(); searchPath = new int[]{};
    }
}
