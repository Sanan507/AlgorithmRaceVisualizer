...
public class HeapSortModel extends AlgorithmModel {
    ...
    public HeapSortModel() { super("Heap Sort", "O(n log n)"); }

    private void heapify(int n, int i) {
        int largest = i, l = 2 * i + 1, r = 2 * i + 2;
        if (l < n) { addComparison(); if (array[l] > array[largest]) largest = l; }
        if (r < n) { addComparison(); if (array[r] > array[largest]) largest = r; }
        if (largest != i) { swap(i, largest); heapify(n, largest); }
    }
